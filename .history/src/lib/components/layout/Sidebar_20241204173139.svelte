<script lang="ts">
    ...
    selectedModels.update((models) => models.filter((model) => model.id !== modelId));
  };
let usedModels=[];
let selectedModel = '';  // 初始为一个空字符串，表示没有选择模型
	...
	{/if}
		...
		<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				...
				>
					...
					<div class="flex self-center">
						<div class=" self-center font-medium text-sm font-primary">{selectedModels}</div>
					</div>
				</a>
			</div>
			{/each}
			...
			<ul>
			  {#each $selectedModels as model}
				<li>
				  ...
				  <button on:click={() => handleRemoveModel(model.id)}>Remove</button>
				</li>
			  {/each}
			</ul>
