<script lang="ts">
	...
	import { onMount, getContext, tick, onDestroy ,afterUpdate} from 'svelte';
	import { WEBUI_NAME, config, models as _models, selectedModels} from '$lib/stores';
	import {
		createNewModel,
		deleteModelById,
...
//处理隐藏模型的功能
	const hideModel = (modelId: string) => {
  hiddenModels.update(models => {
    const newModels = [...models, modelId];
    saveHiddenModels(newModels); // 更新 localStorage
    return newModels;
  });
};
...
//角色模型在侧边栏的展示
  ...
  afterUpdate(() => {
    ...
    if (modelsValue !== selectedModelName) {
      selectedModelName = modelsValue;
      console.log('更新后的模型:', selectedModelName);
    }
  });
  const ModelHandler = (modelName) => {
  goto(`/?models=${encodeURIComponent(modelName)}`, { replaceState: false, force: true });
};
	onMount(async () => {
		loadHiddenModels(); // 加载隐藏的模型

		if (token) {
      try {
        // 获取工作区模型列表
        const workspaceModels = await getWorkspaceModels(token);
        // 更新 store 中的模型数据
		await _models.set(await getModels(localStorage.token));
		 const gptsmodels = await getWorkspaceModels(localStorage.token);
		console.log("model:",gptsmodels)
		gptsModels.set(gptsmodels); // 更新 store
      } catch (error) {
    ...
    console.log('初始化模型:', selectedModelName);
		...
		dropZone?.addEventListener('dragleave', onDragLeave);
		    ...
		    // 读取已使用的模型信息
	const storedModels = sessionStorage.selectedModels;
	console.log("sessionStorage:",sessionStorage)
    if (storedModels) {
      usedModels = JSON.parse(storedModels);
  ...
  // 直接使用响应式绑定来处理模型选择
    $: if (selectedModels) {
    // 当 selectedModel 更新时，做一些处理，比如加载该模型的历史记录
...
>
	...
	>
		...
		{/if}
		{#if $user?.role === 'admin' || $user?.permissions?.Masks?.models  }
		<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
			<a
				class="flex-grow flex space-x-3 rounded-lg px-2 py-[7px] hover:bg-gray-100 dark:hover:bg-gray-900 transition"
				href="/Masks"
	...
	{/if}
		...
		{#if (!$hiddenModels.includes(model.id))} <!-- 只有不在隐藏列表中的模型才会显示 -->
		...
		<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				...
				<!-- 添加隐藏按钮 -->
				<div class="invisible group-hover:visible from-gray-100 dark:from-gray-950 absolute right-0 top-[4px] py-1 pr-0.5 mr-1.5 pl-5 bg-gradient-to-l from-80% to-transparent">
					<div class="flex self-center space-x-1 z-10">
						<button aria-controls="8eTCx0aJWi" aria-expanded="false" data-state="closed" id="yD3PQRPq-U" tabindex="0" data-melt-dropdown-menu-trigger="" data-menu-trigger="" type="button">
							<div aria-label="更多" class="flex"><button aria-label="Chat Menu" class="self-center dark:hover:text-white transition">
				...
				<button
				class="ml-2 p-1.5 bg-gray-300 rounded-full hover:bg-gray-400 text-gray-700"
				on:click={() => hideModel(model.id)}
			  >
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-[1rem]">
