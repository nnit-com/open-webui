...
let selfModels='';
	...
	$: {
		...
		selfModels=Allmodels.filter((model) => model.name=='PersonalAssistant');
...
//角色模型在侧边栏的展示
  ...
  goto(`/g/?models=${encodeURIComponent(modelName)}`, { replaceState: false, force: true });
};
	onMount(async () => {
		if (token) {
      try {
	...
	>
		...
		<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
			...
			<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				...
				<a
					class="flex-grow flex space-x-3 rounded-lg px-2 py-[7px] hover:bg-gray-100 dark:hover:bg-gray-900 transition"
					href={`/g/?models=${encodeURIComponent(KITNModels[0]?.id)}`}
				>
				<div class="self-center">
					<button on:click={() => { selectedModelIdx = modelIdx; }}>
						<img
...
<!-- 个人助手 -->
			<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				...
				<a
					class="flex-grow flex space-x-3 rounded-lg px-2 py-[7px] hover:bg-gray-100 dark:hover:bg-gray-900 transition"
					href={`/g/?models=${encodeURIComponent(selfModels[0]?.id)}`}
				>
				<div class="self-center">
					<button on:click={() => { selectedModelIdx = modelIdx; }}>
						<img
