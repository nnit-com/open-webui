...
//角色模型在侧边栏的展示
	...
	const ModelHandler = async (model) => {
		goto('/?models=' + encodeURIComponent(model.name));
	};
	   // 响应查询参数变化
	   let selectedModel = ''; // 保存当前选中的模型

	   $: {
    const model = $page.url.searchParams.get('models');
    if (model !== selectedModel) {
      selectedModel = model;
      // 根据模型值加载相应的内容或执行其他操作
      console.log('模型发生变化:', selectedModel);
    }
  }
	onMount(async () => {
		if (token) {
