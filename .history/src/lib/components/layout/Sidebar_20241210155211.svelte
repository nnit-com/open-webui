...
let KITNModels='';
	...
	//响应式订阅模型数据
	$: {
		// 仅显示 `is_showInSidebar` 为 true 的模型
		sidebarModels = models.filter((model) => model.is_showInSidebar);
		console.log('Filtered sidebarModels:', sidebarModels);
	}
	$: {
		// 仅显示 `is_showInSidebar` 为 true 的模型
		KITNModels = Allmodels.filter((model) => model.name=='KITN');
		console.log('KITN:', KITNModels.id);
	}
	...
	>
		...
		<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
			...
			<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				<a
					class="flex-grow flex space-x-3 rounded-lg px-2 py-[7px] hover:bg-gray-100 dark:hover:bg-gray-900 transition"
					href={`/?models=${encodeURIComponent(KITNModels[0]?.id)}`}
				>
				<div class="self-center">
					<button on:click={() => { selectedModelIdx = modelIdx; }}>
						<img
						crossorigin="anonymous"
						src={KITNModels?.info?.meta?.profile_image_url ??
							($i18n.language === 'dg-DG'
							  ? `/doge.png`
							  : `${WEBUI_BASE_URL}/static/favicon.png`)}
						class="rounded-full border-[1px] border-gray-200 dark:border-none"
					...
					<div class="flex self-center">
						<div class=" self-center font-medium text-sm font-primary">{KITNModels.name}</div>
					</div>
				</a>
			</div>
