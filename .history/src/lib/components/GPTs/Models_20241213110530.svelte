...
// 根据 searchValue（搜索值）对 models（模型列表）进行过滤，然后将过滤后的结果赋值给 filteredModels
	...
	// 切换模型在侧边栏中的显示状态
	async function toggleSidebarState(model) {
		try {
			// 调用后端 API 更新状态（根据需求，更新后端状态）
			//await showModelInSidebarById(localStorage.token, model.id);

			// 更新本地 store 和 localStorage
			userPreferences.update((prefs) => {
				let updatedSidebarMasks = [...prefs.sidebarMasks];
				if (model.is_showInSidebar) {
					updatedSidebarMasks.push(model.id);

					// 如果模型已在侧边栏，移除
				} else {
					// 否则添加到侧边栏
					updatedSidebarMasks = updatedSidebarMasks.filter(id => id !== model.id);
			...
			// 提示用户操作成功
			toast.success(`Model ${model.name} has been ${model.is_showInSidebar ? 'added to' : 'removed from'} the sidebar.`);
		} catch (error) {
			toast.error(`Failed to update sidebar state for model ${model.name}.`);
			console.error(error);
		}
	}
	const hideModelHandler = async (model) => {
		...
		if (res) {
			toast.success(
				$i18n.t(`Mask {{name}} is now {{status}}`, {
					name: info.id,
		...
		<div class=" flex flex-1 items-center w-full space-x-2">
			<div class="flex flex-1 items-center">
				...
				<input
					...
					placeholder={$i18n.t('Search Mask')}
				/>
			</div>
...
<!-- 
							...
							<div class="ml-1">
								<Tooltip content={model.is_showInSidebar ? $i18n.t('Disabled in Sidebar') : $i18n.t('Enabled in Sidebar')}>
									<Switch
										bind:state={userPreferences.sidebarMasks.includes(model.id)}
										on:change={() => toggleSidebarState(model)}
	...
	{#if $user?.role === 'admin'}
		<div class=" flex justify-end w-full mb-3">
			<div class="flex space-x-1">
				...
				>
					<div class=" self-center mr-2 font-medium line-clamp-1 text-xl">{$i18n.t('Import Mask')}</div>
				...
				>
					<div class=" self-center mr-2 font-medium line-clamp-1 text-xl">{$i18n.t('Export Mask')}</div>
