...
// 根据 searchValue（搜索值）对 models（模型列表）进行过滤，然后将过滤后的结果赋值给 filteredModels
	$: if (models) {
		...
		sidebarModels = models.filter(model => $userPreferences.sidebarMasks.includes(model.id));

		filteredModels = models.filter(
			(m) => searchValue === '' || m.name.toLowerCase().includes(searchValue.toLowerCase())
		);
		models.forEach(model => {
			console.log("is_showInSidebar",models)
            // 如果该模型在 sidebarModels 中，则将其 is_showInSidebar 设置为 true
            model.is_showInSidebar = sidebarModels.some(sidebarModel => sidebarModel.id === model.id);
        });
	}
	...
	// 切换模型在侧边栏中的显示状态
		...
		function toggleSidebarState(modelId, is_showInSidebar) {
        userPreferences.update(prefs => {
            let updatedSidebarMasks = [...prefs.sidebarMasks];
            if (is_showInSidebar) {
                updatedSidebarMasks.push(modelId);  // 如果选中，则添加到 sidebar
            } else {
                updatedSidebarMasks = updatedSidebarMasks.filter(id => id !== modelId);  // 否则移除
            }
            prefs.sidebarMasks = updatedSidebarMasks;  // 更新 store
            return prefs;
        });
    }
							...
							<div class="ml-1">
								<Tooltip content={model.is_showInSidebar ? $i18n.t('Enabled in Sidebar') : $i18n.t('Disabled in Sidebar')}>
									<Switch
										...
										on:change={(e) => toggleSidebarState(model, e.detail)}
									/>
									<!-- bind:state={model.is_showInSidebar}
