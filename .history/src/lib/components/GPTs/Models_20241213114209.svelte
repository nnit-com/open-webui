...
<!-- 
							...
							<div class="ml-1">
								<Tooltip content={model.is_showInSidebar ? $i18n.t('Enabled in Sidebar') : $i18n.t('Disabled in Sidebar')}>
									<!-- 在 each 循环内初始化 isInSidebar -->
									{#let isInSidebar = $userPreferences.sidebarMasks.includes(model.id)}
									<Switch
										bind:state={isInSidebar}  
										on:change={(e) => toggleSidebarModel(model.id, e.detail)}
									/>
								{/let}
