<script lang="ts">
	...
	let loaded = false;
    let isInSidebar = $userPreferences.sidebarMasks
...
// 根据 searchValue（搜索值）对 models（模型列表）进行过滤，然后将过滤后的结果赋值给 filteredModels
	...
	// 切换模型在侧边栏中的显示状态
	async function toggleSidebarState(modelId, isShow) {
		try {
			...
			// 提示用户操作成功
			toast.success(`Model ${modelId.name} has been ${modelId.is_showInSidebar ? 'added to' : 'removed from'} the sidebar.`);
		} catch (error) {
			toast.error(`Failed to update sidebar state for model ${modelId.name}.`);
			console.error(error);
		}
	}
...
<!-- 
							...
							<div class="ml-1">
								<Tooltip content={model.is_showInSidebar ? $i18n.t('Enabled in Sidebar') : $i18n.t('Disabled in Sidebar')}>
									{#if $userPreferences.sidebarMasks.includes(model.id)}
                        <Tooltip content="Enabled in Sidebar">
                            <Switch
                                bind:state={true} 
                    ...
                    {:else}
                        <Tooltip content="Disabled in Sidebar">
                            <Switch
                                bind:state={false}  
                                on:change={(e) => toggleSidebarState(model.id, e.detail)}
                            />
