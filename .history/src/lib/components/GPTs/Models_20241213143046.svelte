...
// 根据 searchValue（搜索值）对 models（模型列表）进行过滤，然后将过滤后的结果赋值给 filteredModels
  ...
  function toggleSidebarState(modelId, is_ShowInSidebar) {
    userPreferences.update(prefs => {
      const updatedSidebarMasks == is_ShowInSidebar
        ? [...prefs.sidebarMasks, modelId]  // 如果选中，添加到 sidebarMasks
        : prefs.sidebarMasks.filter(id => id !== modelId);  // 否则从 sidebarMasks 中移除

      prefs.sidebarMasks = updatedSidebarMasks;  // 更新 store 中的偏好设置
      return prefs;
    });
  }
