...
// 根据 searchValue（搜索值）对 models（模型列表）进行过滤，然后将过滤后的结果赋值给 filteredModels
    ...
    // 处理模型的显示状态
    function updateModelsSidebarState() {
        models.update(currentModels => {
            return currentModels.map(model => {
                // 更新每个模型的 is_showInSidebar 状态
    ...
    // 初始化时更新模型显示状态
    updateModelsSidebarState();

    // 当用户偏好变化时更新模型显示状态
    $: $userPreferences.sidebarMasks, updateModelsSidebarState();
  ...
  function toggleSidebarState(modelId, isShow) {
        userPreferences.update(prefs => {
            ...
            if (isShow) {
                updatedSidebarMasks.push(modelId);  // 如果是开启，则添加到显示列表
            } else {
                updatedSidebarMasks = updatedSidebarMasks.filter(id => id !== modelId);  // 否则从列表中移除
            }
